public class DateTime.TimeMap : Gtk.Grid {
    ...
    public override bool draw (Cairo.Context cr) {
        ...
        if (width < BG_WIDTH && width != old_width) {
            ratio = ((double)width/(double)BG_WIDTH);
            background_map_scale = background_map.scale_simple (width, (int)(ratio*(double)BG_HEIGHT), Gdk.InterpType.BILINEAR);
            selected_scale = selected.scale_simple (width, (int)(ratio*(double)BG_HEIGHT), Gdk.InterpType.BILINEAR);
            queue_resize_no_redraw ();
        } else if (width >= BG_WIDTH) {
            ratio = 1;
            background_map_scale = background_map;
    ...
    public override void get_preferred_height (out int minimum_height, out int natural_height) {
        ...
        natural_height = (int)(ratio*(double)BG_HEIGHT);
    }
